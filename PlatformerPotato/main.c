...
static void RegeneratePlatforms()
{
	...
	for (int n = 0; n < 10; n++)
	{
		platforms[n] = (Rectangle){n * 100.0f, 400, 100, 20 };
	}

	for (int n = 10; n < NUM_PLATFORMS; n++)
	{
		platforms[n] = (Rectangle){ (float)GetRandomValue(0, 800), -75.0f * n + 1000, 100 , 1 };
	}
}
...
{
	camera.offset = (Vector2){ W * 0.5f, H * 0.7f };
	camera.target = (Vector2){ lerp(camera.target.x, player.x, 0.05f), lerp(camera.target.y, player.y, 0.1f) };
...
int main()
{
	...
	idleSheet = LoadTexture("idle_sheet.png");
	...
	while (!WindowShouldClose())
	{
		...
		if (titlescreen)
		{
			...
			DrawTextEx(vcr, "VERTIGO", (Vector2){ camera.target.x - camera.offset.x + 30, camera.target.y - camera.offset.y + 30 }, 32, 0, BLACK);
			const char* instructions1 = "USE THE PLATFORMS TO SCALE THE BUILDING.";
			DrawTextEx(vcr, instructions1, (Vector2) { camera.target.x - camera.offset.x + 10, camera.target.y}, 12, 0, BLACK);
			const char* instructions2 = "THE COUNTDOWN TIMER WILL BE EXTENDED AS LONG AS YOU KEEP REACHING NEW HEIGHTS.";
			DrawTextEx(vcr, instructions2, (Vector2) { camera.target.x - camera.offset.x + 10, camera.target.y + 12}, 12, 0, BLACK);
			const char* instructions3 = "WHEN THE COUNTDOWN REACHES ZERO, IT'S GAME OVER.";
			DrawTextEx(vcr, instructions3, (Vector2) { camera.target.x - camera.offset.x + 10, camera.target.y + 24 }, 12, 0, BLACK);
			const char* instructions4 = "YOU CAN REGENERATE THE PLATFORMS TO POSSIBLY CREATE A BETTER PATH.";
			DrawTextEx(vcr, instructions4, (Vector2) { camera.target.x - camera.offset.x + 10, camera.target.y + 36 }, 12, 0, BLACK);
			const char* instructions5 = "[PRESS ENTER TO PLAY]";
			DrawTextEx(vcr, instructions5, (Vector2) { camera.target.x - camera.offset.x + 10, camera.target.y + 48 }, 16, 0, BLACK);

			DrawTexturePro(
				jumpSheet, (Rectangle){ 0, frame * 80.0f, 64, 80 }, (Rectangle){ 100, 100, 128, 160 }, (Vector2){0, 0}, 0, Fade(GRAY, 0.5f));
		...
		else
		{
			...
			{
				DrawTexturePro(
					highRise, (Rectangle) { 0.0f, 0.0f, 2048.0f, 2048.0f }, (Rectangle) { 0.0f, -1024 * i + 74, 1024.0f, 1024.0f }, (Vector2) { 0, 0 }, 0, DARKGRAY);
			}
...

			{
				...
				{
					DrawTexturePro(*sheetToUse, (Rectangle){0, frame * 80.0f, 64, 80}, playerRect, (Vector2){ 0,0 }, 0, WHITE);
				}
				else
				{
					DrawTexturePro(*sheetToUse, (Rectangle){64, frame * 80.0f, -64, 80}, playerRect, (Vector2){0,0}, 0, WHITE);
				}
			}
		...
		
			{
				...
				DrawTextEx(vcr, TextFormat("(%0.0f FT)", currentFT), (Vector2){ player.x - 30, player.y - 100 }, 12, 1, RAYWHITE);
				...
				DrawTextEx(vcr, TextFormat("HIGHEST: %i ft", heightPB), 
					(Vector2){ camera.target.x - camera.offset.x, camera.target.y - camera.offset.y}, 20, 1, colourPB);
			
				DrawTextEx(vcr, "(PRESS ENTER TO REGENERATE PLATFORMS)", 
					(Vector2){ camera.target.x, camera.target.y - camera.offset.y }, 10, 1, WHITE);
			}
